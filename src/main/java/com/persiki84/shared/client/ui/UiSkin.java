...
public final class UiSkin {
    ...
    public static void button(GuiGraphics graphics, float x, float y, float width, float height,
        ...
        try {
            surface(graphics, bx, by, w, h, active, focus, pressed);
            label(graphics, label, bx, by, w, h, active || lit, focus, labelScale * (1.0f - squash));
    ...
    public static void icon(GuiGraphics graphics, float x, float y, float width, float height,
        ...
        surface(graphics, x, y, width, height, active, focus, 0.0f);
    }
    ...
    public static void slider(GuiGraphics graphics, float x, float y, float width, float height,
        ...
        surface(graphics, x, y, width, height, active, focus, 0.0f);
        label(graphics, label, x, y, width, Math.max(height * 0.5f, height - TRACK_BOTTOM), active, focus, labelScale);
    ...
    private static void surface(GuiGraphics graphics, float x, float y, float width, float height,
                                boolean active, float focus, float pressed) {
        float radius = Math.min(UiGlassStyle.radiusPanel() + 2.0f, height / 2.0f);
        if (!active) {
            UiGlass.sunken(graphics, x, y, width, height, radius, 0.85f);
            return;
        }
        float shimmer = Math.max(0.0f, pressed);
        UiGlass.panel(graphics, x, y, width, height, radius, 1.0f, focus * 0.85f + shimmer * 0.45f);
        sweep(graphics, x, y, width, height, radius, Math.max(focus, shimmer * 1.4f));
    }
    ...
    private static void label(GuiGraphics graphics, Component label, float x, float y, float width, float height,
                              boolean active, float focus, float scale) {
        if (label == null || label.getString().isEmpty()) return;
        int color = active ? UiTheme.mix(UiAccent.text(), UiTheme.WHITE, focus) : UiAccent.textFaint();
